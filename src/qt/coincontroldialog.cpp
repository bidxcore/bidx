...
{
    GUIUtil::setClipboard(BIDXUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BIDXUnits::BIDX;
    ...
    l2->setText(BIDXUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(BIDXUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(BIDXUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(BIDXUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BIDXUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BIDXUnits::format(nDisplayUnit, nSum));
