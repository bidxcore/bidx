...
static UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print BIDX address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CBIDXAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                CBIDXAddress address(mn.pubKeyCollateralAddress.GetID());
...
//            return mnodeman.CountEnabled(MIN_PRIVATESEND_PEER_PROTO_VERSION);
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.vin.prevout.ToString()));
        obj.push_back(Pair("payee",         CBIDXAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CBIDXAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
    ...
    if (strCommand == "decode")
    {
        ...
        for(CMasternodeBroadcast& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CBIDXAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CBIDXAddress(mnb.pubKeyMasternode.GetID()).ToString()));
